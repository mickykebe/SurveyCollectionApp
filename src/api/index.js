...
const Auth = {
  ...
  login: (username, password) =>
    requests
      ...
      .then(data => mapper.login(data)),
  memberRegister: user =>
    superagent
      .post(`${BASE_URL}/profiles/`, user)
      .then(responseBody)
      .then(data => mapper.register(data)),
  adminRegister: company =>
    superagent
      .post(`${BASE_URL}/companies/`, company)
      .then(responseBody)
      .then(data => mapper.adminRegister(data))
};

const Surveys = {
  mine(published, offset = 0) {
    return requests
      .get(`/surveys/mine/?query={"active":${published}}&offset=${offset}`)
...
const SurveyResponses = {
  all(id, offset = 0) {
    return requests
      .get(`/surveys/${id}/filled-surveys/?offset=${offset}`)
      .then(data => mapper.surveyResponses(data));
  },
  allFormat(id, format = "csv") {
    return superagent
      .get(`${BASE_URL}/surveys/${id}/export/?format=${format}`)
      .use(tokenPlugin)
      .responseType("blob")
      .then(responseBody);
  }
...
const Profiles = {
  all(companyId) {
    return requests.get(
      `/profiles/?query={"company__uuid":"${companyId}"}&limit=0`
    );
  },
  get(id) {
