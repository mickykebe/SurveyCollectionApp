...
export default combineReducers({
  byId,
  responsesStatus: createKeyWrappedReducer(action => action.survey)(
    combineReducers({
      status: asyncStatus(
        ACTION_RESPONSES_FETCH_REQUEST,
        ACTION_RESPONSES_FETCH_SUCCESS,
...
export const getSurveyResponse = (state, id) => state.byId[id];
export const getIsFetchingSurveyResponses = (state, surveyId) =>
  !!(
    state.responsesStatus[surveyId] &&
    state.responsesStatus[surveyId].status.inProgress
  );
export const getSurveyResponsesFetchErrors = (state, surveyId) =>
  (state.responsesStatus[surveyId] &&
    state.responsesStatus[surveyId].status.errors) ||
  null;
export const getSurveyResponsesCount = (state, surveyId) =>
  (state.responsesStatus[surveyId] &&
    state.responsesStatus[surveyId].pagination.count) ||
  0;
export const getSurveyResponsesNext = (state, surveyId) =>
  (state.responsesStatus[surveyId] &&
    state.responsesStatus[surveyId].pagination.next) ||
  0;
export const getSurveyResponseAnswer = (state, responseId, questionId) => {
  const { answers = [] } = state.byId[responseId] || {};
  return answers.find(answer => answer.question === questionId);
};
