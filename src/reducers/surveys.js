...
const createList = (activeStatus) => {
  ...
  return combineReducers({
    ids,
    pagination: pagination(activeStatus ? actions.ACTION_SURVEY_PUBLISHED_FEED_FETCH_SUCCESS : actions.ACTION_SURVEY_PENDING_FEED_FETCH_SUCCESS),
    status: asyncStatus(
      activeStatus ? actions.ACTION_SURVEY_PUBLISHED_FEED_FETCH_REQUEST : actions.ACTION_SURVEY_PENDING_FEED_FETCH_REQUEST,
      activeStatus ? actions.ACTION_SURVEY_PUBLISHED_FEED_FETCH_SUCCESS : actions.ACTION_SURVEY_PENDING_FEED_FETCH_SUCCESS,
      activeStatus ? actions.ACTION_SURVEY_PUBLISHED_FEED_FETCH_FAIL : actions.ACTION_SURVEY_PENDING_FEED_FETCH_FAIL,
    )
  });
}
...
export default combineReducers({
  ...
  responsesById,
  create: asyncStatus(actions.ACTION_SURVEY_CREATE_REQUEST, actions.ACTION_SURVEY_CREATE_SUCCESS, actions.ACTION_SURVEY_CREATE_FAIL),
  fetch: createKeyWrappedReducer(
      action => action.id,
    )(asyncStatus(actions.ACTION_SURVEY_FETCH_REQUEST, actions.ACTION_SURVEY_FETCH_SUCCESS, actions.ACTION_SURVEY_FETCH_FAIL)),
  update: createKeyWrappedReducer(
      action => action.id,
    )(asyncStatus(actions.ACTION_SURVEY_UPDATE_REQUEST, actions.ACTION_SURVEY_UPDATE_SUCCESS, actions.ACTION_SURVEY_UPDATE_FAIL)),
  delete: createKeyWrappedReducer(
      action => action.id,
    )(asyncStatus(actions.ACTION_SURVEY_DELETE_REQUEST, actions.ACTION_SURVEY_DELETE_SUCCESS, actions.ACTION_SURVEY_DELETE_FAIL)),
});
